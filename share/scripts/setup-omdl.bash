...
# assign after declare to preserve exist status
declare kernel
kernel=$(uname -s)
...
declare setup_amu_url="https://raw.githubusercontent.com/royasutton/openscad-amu/master/share/scripts/setup-amu.bash"
...
function update_prerequisite_list() {
  ...
  # Packages common to all platforms (apt/cygwin names; macOS names may differ)
  local packages_Common="bash wget git make unzip openscad"

  # Platform-specific additions (space-separated)
  local packages_Linux=""

  local packages_CYGWIN_NT=""

  # On macOS the openscad cask is handled by prerequisite_install_openscad.Darwin;
  # list only brew-installable CLI deps here (openscad removed from common for Darwin).
  local packages_Darwin="coreutils gnu-sed wget git make unzip"
...
function prerequisite_check.Darwin() {
  # Returns 0 if the brew formula/cask is installed, 1 otherwise.
...
function prerequisites_install.Darwin() {
  # Ensure Homebrew is present before attempting any install.
...
function prerequisite_install_openscad.Darwin() {
  ...
  case "${pkg}" in
    ...
    openscad-nightly)
      # The nightly/snapshot build is distributed as a plain .dmg; fall back to
      # the stable cask and warn the user.
...
function prerequisites_install() {
  ...
  if [[ -n "${packages_missing}" ]] ; then
    ...
    # Separate exception packages (openscad variants) from regular ones so
    # the regular packages can be installed in a single package-manager call.
    ...
    # Install all regular packages in one batch invocation.
...
function toolchain_prepare() {
  ...
  shell_cmd_path=$(command -v "${test_cmd_name}-${amu_version}" 2>/dev/null)
